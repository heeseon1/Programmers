...
- 기존 반복문
  - 각 동전(쿼터, 다임, 니켈, 페니)의 값을 이용해 반복문 안에서 차례대로 나누고 몫을 구해 String 값으로 변환 후 출력
  - 메모리: 16348KB,	시간: 148 ms
    
- 향상된 반복문
  - 쿼터, 다임, 니켈, 페니의 값을 배열(coins)에 저장
  - 향상된 for문(for-each)을 사용하여 반복 처리
  - StringBuilder를 활용하여 문자열을 효율적으로 생성 및 출력
